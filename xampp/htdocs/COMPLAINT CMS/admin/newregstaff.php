...
function validatestaffaddm(x){
  var addm=x;
  alert(x);
  $.post("validatestaff.php",{addm:addm},function(result){
    alert(result);
    if (result==1){

      document.getElementById("error2").innerHTML = 'AdmissionNumber exists , try logging in with it';
...
function validatestaffUN(x){
  var UN=x;
  alert(x);
  $.post("validatestaff.php",{UN:UN},function(result){
    alert(result);
    if (result==1){

      document.getElementById("error").innerHTML = 'UserName already exists , try logging in with it';
      // document.getElementById(id).attribute = new value
      // document.getElementById('emailreg').innerHTML = 'email already registered , use another email';
    }
    else{
        ...
        <form class="form-floating ng-pristine ng-invalid ng-invalid-required ng-valid-email ng-valid-url ng-valid-pattern" form role="form" action="signupbyadmin.php" method="get">
          <fieldset>
            <div class="form-group">
              ...
              <input type="text" onblur="validatestaffUN(this.value)" class="form-control1 ng-invalid ng-invalid-required ng-touched" name="UserName" placeholder="User Name" required="">
              <p style="color:red;" id="error"></p>
            </div>
            <div class="form-group">
              <label class="control-label">Department</label>
                <p>1.Bus 2.Water 3.teaching 4.Sports </p>
              <input id="" type="text" class="form-control1 ng-invalid ng-valid-email ng-invalid-required ng-touched" name="Department" placeholder="Department" required="">
            ...
            <div class="form-group">
              <label  class="control-label">AdmissionNumber</label>
              <input onblur='validatestaffaddm(this.value)' type="text" class="form-control1" name="AddmNUmber" placeholder="AddmNUmber" required="">
